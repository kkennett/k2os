...
END_A32_PROC(ResetVector)
    ...
    cmp     r0, #0
    beq     Is_Cpu0
    b       BSP_NoStack_NoInts_PhysicalStartCpuN
     
Is_Cpu0
    b       BSP_NoStack_NoInts_PhysicalStartCpu0


    ALIGN
    LEAF_ENTRY BSP_NoStack_NoInts_PhysicalStartCpu0
        ...
        ldr r2, =(VIRTARM_PHYSSIZE_BOOTROM / 16)
...
blRamCopy
        ...
        ldr     r0, =VIRTARM_PHYSSIZE_BOOTROM
        ldr     r1, =VIRTARM_PHYSADDR_RAMBANK
...
blCodeInRam
        ;---------------------------------------------------------------
        ; executing in RAM.  jump to common bootloader
        ;---------------------------------------------------------------    
        add r0, pc, #BootMapTable - (. + 8)    ; Pointer to OEMAddressTable for bootloader
        b  BootMMUStart

    ENTRY_END BSP_NoStack_NoInts_PhysicalStartCpu0
